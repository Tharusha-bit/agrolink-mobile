...
const KpiCard = ({ label, value, unit, delta, positive, sparkData, icon, iconColor }: KpiCardProps) => {
    ...
    <View style={[kpi.card, SH.sm]}>
      ...
      <View style={kpi.topRow}>
        <View style={[kpi.iconBox, { backgroundColor: iconBg }]}>
          <MaterialCommunityIcons name={icon as any} size={18} color={iconColor ?? C.forest} />
        </View>
...
const kpi = StyleSheet.create({
  card:      { backgroundColor: C.cardBg, borderRadius: 22, padding: 18, width: 168, marginRight: 14 },
  ...
  label:     { fontSize: 11, fontWeight: '700', color: C.textMuted, letterSpacing: 0.6, textTransform: 'uppercase', flex: 1 },
  ...
  deltaPill: { flexDirection: 'row', alignItems: 'center', borderRadius: 30, paddingHorizontal: 8, paddingVertical: 4, gap: 3 },
...
const ActionCard = ({ icon, label, sublabel, iconBg, iconColor, primary = false, onPress }: ActionCardProps) => (
  <TouchableOpacity
    style={[ac.card, primary && ac.cardPrimary, SH.sm]}
    onPress={onPress}
    activeOpacity={0.78}
  >
    <View style={[ac.iconWrap, { backgroundColor: primary ? 'rgba(255,255,255,0.22)' : iconBg }]}>
      <MaterialCommunityIcons name={icon as any} size={24} color={primary ? C.white : iconColor} />
    </View>
    <Text style={[ac.label, primary && ac.labelPrimary]}>{label}</Text>
    <Text style={[ac.sublabel, primary && ac.sublabelPrimary]} numberOfLines={2}>{sublabel}</Text>
  </TouchableOpacity>
);

const ac = StyleSheet.create({
  card:          { backgroundColor: C.cardBg, borderRadius: 20, padding: 18, flex: 1, minHeight: 130 },
...
const TimelineRow = ({ icon, dotColor, title, subtitle, time, amount, amountPositive, isLast = false }: TimelineRowProps) => (
  <View style={tr.wrap}>
    {/* Spine */}
    <View style={tr.spine}>
      <View style={[tr.dot, { backgroundColor: dotColor + '18', borderWidth: 2, borderColor: dotColor }]}>
    ...
    <View style={tr.body}>
      <View style={tr.topRow}>
        ...
        {amount && (
          <Text style={[tr.amount, { color: amountPositive ? C.gain : C.loss }]}>{amount}</Text>
        )}
      </View>
...
const tr = StyleSheet.create({
  ...
  line:    { width: 2, flex: 1, backgroundColor: C.borderLight, marginTop: 5, minHeight: 22 },
  ...
  title:   { fontSize: 13.5, fontWeight: '700', color: C.text, flex: 1, marginRight: 8 },
...
// ─── Section Label ─────────────────────────────────────────────────────────────
const SectionLabel = ({ title, action, onAction }: { title: string; action?: string; onAction?: () => void }) => (
  <View style={lbl.row}>
...
const lbl = StyleSheet.create({
  row:       { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginHorizontal: 20, marginBottom: 14 },
  leftGroup: { flexDirection: 'row', alignItems: 'center' },
  pill:      { width: 4, height: 18, borderRadius: 2, backgroundColor: C.gold, marginRight: 10 },
  title:     { fontSize: 12, fontWeight: '900', color: C.textSub, letterSpacing: 1.1, textTransform: 'uppercase' },
...
const StrengthBar = ({ percent }: { percent: number }) => {
  ...
  const TIPS   = ['Upload photo', 'Verify NIC', 'Link bank', 'Add skills', 'Full address'];
    ...
    <View style={str.wrap}>
      ...
      <View style={str.segRow}>
        {Array.from({ length: STEPS }).map((_, i) => (
          <View
            key={i}
            style={[str.seg, i < filled ? { backgroundColor: C.forest } : { backgroundColor: C.borderLight }, i < STEPS - 1 && { marginRight: 5 }]}
          />
        ))}
      </View>
      {nextTip && (
        <View style={str.tipChip}>
          <MaterialCommunityIcons name="lightning-bolt" size={12} color={C.gold} />
          <Text style={str.tipText}>Next step: <Text style={{ fontWeight: '700', color: C.forest }}>{nextTip}</Text></Text>
        </View>
      )}
    </View>
  );
...
const str = StyleSheet.create({
  ...
  tipChip:  { flexDirection: 'row', alignItems: 'center', gap: 5, backgroundColor: C.forestGhost, borderRadius: 20, paddingHorizontal: 12, paddingVertical: 6, alignSelf: 'flex-start', borderWidth: 1, borderColor: C.border },
  tipText:  { fontSize: 12, color: C.textMuted },
});

// ─── Main Screen ──────────────────────────────────────────────────────────────
const KPI_DATA: KpiCardProps[] = [
  { label: 'Portfolio',     value: '2.4M', unit: 'LKR', delta: '+8.2%', positive: true,  sparkData: [55, 60, 58, 70, 68, 80, 95], icon: 'cash-multiple',      iconColor: C.forest     },
  { label: 'Avg Return',    value: '18.4', unit: '%',   delta: '+2.1%', positive: true,  sparkData: [38, 44, 42, 52, 56, 62, 68], icon: 'trending-up',         iconColor: C.gain       },
  { label: 'Investments',   value: '6',    unit: '',    delta: '-1',    positive: false, sparkData: [8, 7, 9, 8, 7, 7, 6],        icon: 'sprout',              iconColor: C.pending    },
  { label: 'Trust Score',   value: '4.8',  unit: '★',   delta: '+0.2',  positive: true,  sparkData: [4.2,4.3,4.5,4.5,4.6,4.7,4.8], icon: 'shield-star',       iconColor: C.gold       },
];

const ACTIVITIES: TimelineRowProps[] = [
  { icon: 'cash-plus',      dotColor: C.gain,    title: 'Return Credited',        subtitle: 'Suriyakumar Paddy Farm — Season 2',     time: '2 hrs ago',   amount: '+LKR 14,000', amountPositive: true  },
  { icon: 'sprout',         dotColor: C.pending, title: 'New Opportunity',        subtitle: 'Priya Devi Organic Vegetables, Jaffna',  time: 'Yesterday',   amount: 'LKR 30,000',  amountPositive: false },
  { icon: 'alert-outline',  dotColor: C.pending, title: 'Weather Alert',          subtitle: 'Monsoon risk flagged for North Province', time: '2 days ago'                                                },
  { icon: 'shield-check',   dotColor: C.info,    title: 'Identity Verified',      subtitle: 'NIC confirmed by AgroLink trust system', time: '3 days ago'                                                },
  { icon: 'bank-transfer-out', dotColor: C.loss, title: 'Withdrawal Processing',  subtitle: 'LKR 50,000 to Commercial Bank — pending', time: '4 days ago', amount: '-LKR 50,000', amountPositive: false },
];
    ...
    <View style={s.root}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 110 }}>
        ...
        */}
          ...
          <View style={s.identityRow}>
            ...
            >
              <View style={s.avatarRing}>
                <View style={s.avatarCircle}>
                  <MaterialCommunityIcons name="account" size={42} color={C.forest} />
                </View>
              </View>
              {/* Verified badge */}
            ...
            <View style={s.identityBlock}>
              ...
              <View style={s.rolePill}>
                <MaterialCommunityIcons name="trending-up" size={11} color={C.gold} style={{ marginRight: 4 }} />
              ...
              <View style={s.portfolioHero}>
                ...
                <View style={s.portfolioDelta}>
                  <MaterialCommunityIcons name="trending-up" size={12} color={C.gain} />
        ...
        {/* ── PROFILE STRENGTH ─────────────────────────────────────────────── */}
        <SectionLabel title="Profile Strength" action="Improve" onAction={() => router.push('/profile/edit')} />
        ...
        <View style={s.footerRow}>
          <TouchableOpacity style={s.footerBtn} onPress={() => router.push('/profile/edit')} activeOpacity={0.8}>
            <MaterialCommunityIcons name="account-edit-outline" size={17} color={C.forest} style={{ marginRight: 7 }} />
            <Text style={s.footerBtnText}>Edit Profile</Text>
          </TouchableOpacity>
          <View style={s.footerGap} />
          <TouchableOpacity style={s.footerBtn} onPress={() => router.push('/profile/security')} activeOpacity={0.8}>
            <MaterialCommunityIcons name="shield-lock-outline" size={17} color={C.forest} style={{ marginRight: 7 }} />
...
const s = StyleSheet.create({
  ...
  avatarRing:     { width: 78, height: 78, borderRadius: 39, borderWidth: 2.5, borderColor: 'rgba(255,255,255,0.5)', justifyContent: 'center', alignItems: 'center' },
  avatarCircle:   { width: 70, height: 70, borderRadius: 35, backgroundColor: 'rgba(255,255,255,0.92)', justifyContent: 'center', alignItems: 'center' },
  verifiedBadge:  { position: 'absolute', top: -4, right: -4, backgroundColor: C.forest, borderRadius: 12, padding: 2, borderWidth: 1.5, borderColor: 'rgba(255,255,255,0.5)' },
  camDot:         { position: 'absolute', bottom: -2, left: 0, width: 22, height: 22, borderRadius: 11, backgroundColor: C.forestMid, justifyContent: 'center', alignItems: 'center', borderWidth: 2, borderColor: C.forest },

  identityBlock:  { flex: 1 },
  greeting:       { fontSize: 12, color: 'rgba(255,255,255,0.6)', fontWeight: '500', marginBottom: 2 },
  investorName:   { fontSize: 22, fontWeight: '900', color: C.white, letterSpacing: -0.5, marginBottom: 8 },
  rolePill:       { flexDirection: 'row', alignItems: 'center', backgroundColor: 'rgba(201,168,76,0.2)', borderRadius: 20, paddingHorizontal: 10, paddingVertical: 4, alignSelf: 'flex-start', borderWidth: 1, borderColor: 'rgba(201,168,76,0.35)', marginBottom: 14 },
  roleText:       { fontSize: 10.5, color: C.gold, fontWeight: '700', letterSpacing: 0.2 },

  portfolioHero:  { backgroundColor: 'rgba(255,255,255,0.1)', borderRadius: 16, padding: 12, borderWidth: 1, borderColor: 'rgba(255,255,255,0.18)' },
  portfolioLabel: { fontSize: 10, fontWeight: '700', color: 'rgba(255,255,255,0.55)', letterSpacing: 0.8, textTransform: 'uppercase', marginBottom: 3 },
  portfolioValue: { fontSize: 20, fontWeight: '900', color: C.white, letterSpacing: -0.5, marginBottom: 4 },
  portfolioDelta: { flexDirection: 'row', alignItems: 'center', gap: 4 },
  portfolioDeltaText: { fontSize: 11.5, fontWeight: '700', color: C.gain },

  /* KPI */
  kpiStrip:   { paddingLeft: 20, paddingRight: 8, paddingBottom: 2, marginBottom: 24 },
  ...
  /* FEED */
  feedCard:   { backgroundColor: C.cardBg, marginHorizontal: 20, borderRadius: 22, paddingTop: 20, paddingBottom: 4, marginBottom: 24 },

  /* STRENGTH */
  strengthCard: { backgroundColor: C.cardBg, marginHorizontal: 20, borderRadius: 20, padding: 20, marginBottom: 20 },

  /* FOOTER */
  footerRow:    { flexDirection: 'row', marginHorizontal: 20, marginBottom: 10 },
  footerBtn:    { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', backgroundColor: C.cardBg, borderRadius: 16, paddingVertical: 14, borderWidth: 1.5, borderColor: C.border, ...SH.xs },
  footerGap:    { width: 12 },
